(function($) {
	...
	var LeadPro = {
		...
		Menu: function() {
			...
			var $navigation = $('#navigation.navbar-conference');
			...
			if ($navigation.hasClass('scrollspy')) {
				setTimeout(function() {
					$body.scrollspy({
						target: '#navigation.navbar-conference',
						offset: 71
					});
				}, 100);
		...
		Form: function() {
			...
			$(document).on('focus keypress', '.affa-form-signup.form-conference input[type="text"], .affa-form-signup.form-conference input[type="email"], .affa-form-signup.form-conference input[type="password"]', function() {
				var $input = $(this);
			...
			$(document).on('submit', '.affa-form-signup.form-conference', function() {
				var $form		= $(this);
		...
		Carousel: function() {
			...
			// Clients slider
			$('.carousel-slider.clients-conference-slider').slick({
				arrows: false,
				speed: 300,
				autoplay: true,
				autoplaySpeed: 3000,
		...
		// Countdown functions & settings
		Countdown: function() {

			// Header countdown
			var $header = $('.header-conference .header-countdown');

			$header.countdown({
				until: new Date($header.data('date')),
				layout: $header.html()
			});
		...
		Maps: function() {
			...
			// Map embed
			$('.affa-map-conference .map-embed').each(function() {
				var id = $(this).attr('id');
				var options = {
					div: '#' + id,
					lat: $(this).data('lat'),
